...
package object_methods_pkg;
    ...
    class trans extends uvm_object;
        ...
        function bit do_compare(uvm_object rhs,uvm_comparer comparer);
            ...
            if(addr != t.addr) begin
                do_compare = 0;
                `uvm_warning("COMERR",$sformatf("addr %8x != %8x",addr,t.addr))
            end
            if(data != t.data) begin
                do_compare = 0;
                `uvm_warning("COMERR",$sformatf("data %8x != %8x",data,t.data))
            end
            if(op != t.op) begin
                do_compare = 0;
                `uvm_warning("COMERR",$sformatf("op %s != %s",op,t.op))
            end
            if(name != t.name) begin
                do_compare = 0;
                `uvm_warning("COMERR",$sformatf("name %s != %s",name,t.name))
            end
        endfunction 
    endclass
    ...
    class object_methods_test extends uvm_test;
        ...
        function new(string nmae = "object_methods_test",uvm_component parent = null);
        ...
        task run_phase(uvm_phase phase);
            ...
            if(!is_equal)
                `uvm_warning("CMPERR","t1 != t2")
            else
                `uvm_info("CMPSUCC","t1 = t2",UVM_LOW) 
