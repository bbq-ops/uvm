package chnl_pkg1;
    ...
    class chnl_initiator;
        ...
        class chnl_generator;
            ...
            task send_trans();
                ...
                assert(req.randomize with {ch_id == local::ch_id; pkt_id == local::pkt_id;});
                    else $fatal("channl packet randomize failure");
                $display(req.sprint());
                this.pkt_id++;
                this.req_mb.put(req);
                this.rsp_mb.get(rsp);
                $display(rsp.sprint());
                assert(rsp.rsp)
                    else $error("error response received", $time);
            endtask
        endclass
        ...
        class chnl_root_test;
            chnl_agent agent[3];
            protected name;
            function new(int ntrans = 100,name = "chnl_root_test");
                foreach(agent[i]) begin
                    agent[i] = new($sfotmatf("chnl_agent[%d]",i),i,ntrans);
                end
            endfunction
            this.name = name;
            $display("%s instantiate object",this.name);
            task run();
