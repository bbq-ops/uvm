...
interface chnl_intf(input clk, input rstn);
    ...
    clocking drv_ck @(posedge clk);
        ...
        input ch_margin,ch_ready;
    endclocking
    
    clocking mon_ck @(posedge clk);
        default input #1ns output #1ns;
        input ch_data,ch_valid,ch_margin,ch_ready;
    endclocking 
endinterface
...
module tb2;
  ...
  initial begin
    ...
    tests["fifo_full_test"]  = fifo_full_test;
    
    if($value$plusargs("TESTNAME=%s",name)) begin
        if(tests.exits(name)) begin
        ...
        $display("No configured TESTNAME ,run default");
