...
package mcdf_pkg;
    ...
    class mcdf_refmod;
        ...
        task do_reg_update();
            reg_trans t;  //事务寄存器：存储寄存器的操作地址，数据，命令
            forever begin
                this.reg_mb.get(t);
                if(t.addr[7:4] == 0 && t.cmd == `WRITE) begin
                    regs[t.addr[3:2]].en = t.data[0];
                    regs[t.addr[3:2]].prio = t.data[2:1];
                    regs[t.addr[3:2]].len = t.data[5:3];
                end
                else if(t.addr[7:4] == 1 && t.cmd == `READ) begin
                    regs[t.addr[3:2]].avail = t.data[7:0];
                end 
            end
        endtask
        ...
        task do_packet(int id);
            ...
            forever begin
                ...
                ot.length = len > 3 ? 32 : 4 << len; //输出的len由寄存器确定
                ot.ch_id = id;
                ot.data = new[ot.length];
                
                foreach(ot.data[m]) begin
                    this.in_mbs[id].get(it);  //取出指定输入通道的数据
                    ot.data[m] = it.data;  //输入数据是32bit数据
                end
    ...
    //MCDF检查器：负责比较DUT的实际输出与参考模型的预期输出，验证功能的正确性
    class mcdf_checker;
        local string name;
        local int err_count;
        ...
        function new(string name = "mcdf_checker");
            ...
            foreach(this.refmod.out_mbs[i]) begin
            ...
            this.err_count = 0;
            this.total_count = 0;
            foreach(chnl_count[i]) chnl_count[i] = 0;
        endfunction
        
        function void set_interface(virtual mcdf_intf intf);
            if(intf == null)
                $error("this handle is not have been intantiated");
            else begin
                this.intf = intf;
        ...
        task do_compare();
            ...
            forever begin
                ...
                if(cmp == 0) begin
                    this.err_count++;
                    rpt_pkg::rpt_msg("[COMFALL]",$sformatf("%t %d mcdf monitor output is different reference model output",$time,this.total_count),rpt_pkg::ERROR,rpt_pkg::TOP,rpt_pkg::LOG);
                end
                else begin
    ...
    function void do_report();
        ...
        foreach(this.chnl_count[i]) s = {s,$sformatf("chnlnel[%0d] comparison count : %0d \n",i,this.chnl_count[i])};
        s = {s,$sformatf("total error count : %d \n",this.err_count)};
        foreach(this.chnl_mbs[i]) begin
            if(this.chnl_mbs[i].num() != 0)
                s = {s,$sformatf("WARNING chnl_mbs[%d] is not empty size = %0d",i,this.chnl_mbs[i].num())};
        end
    ...
    class mcdf_env;
        ...
        function new(string name = "mcdf_env");
            ...
            foreach(chnl_agts[i]) begin
                this.chnl_agts[i] = new($sformatf("chnl_agts[%d]",i));
            ...
            $display("%s intantiated and connected objects",this.name);
        endfunction
        
        virtual task run();
            $display($sformatf("*********%s started**********"));
            this.do_config();
            fork
    ...
    class mcdf_base_test;
        ...
        virtual task run();
            ...
            rpt_pkg::rpt_msg("[TEST]",$sformatf("=== %s AT TIME %0t STARTED ===",this.name,$time),rpt_pkg::INFO,rpt_pkg::HIGH);
    ...
    class mcdf_data_consistence_basic_test extends mcdf_base_test;
        ...
        task do_data();
            void'(chnl_gens[0].randomize() with {ntrans == 100;ch_id == 0;data_nidles == 0;pkg_nidles == 1;data_size == 8;});
            
            void'(chnl_gens[1].randomize() with {ntrans == 100;ch_id == 1;data_nidles == 1;pkg_nidles == 4;data_size == 16;});
            
            void'(chnl_gens[2].randomize() with {ntrans == 100;ch_id == 2;data_nidles == 2;pkg_nidles == 8;data_size == 32;});
