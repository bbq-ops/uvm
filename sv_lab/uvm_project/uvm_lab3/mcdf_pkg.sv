...
package mcdf_pkg;
  ...
  class mcdf_refmod extends uvm_component;
    ...
    function new (string name = "mcdf_refmod", uvm_component parent);
      ...
      foreach(in_bgpk_ports[i]) in_bgpk_ports[i] = new($sformatf("in_bgpk_ports[%d]",i),this);
      
      //TODO-2.1 instantiate the TLM fifos
      foreach(out_tlm_fifos[i]) out_tlm_fifos[i] = new($sformatf("out_tlm_fifos[%d]",i),this);
  ...
  class mcdf_checker extends uvm_scoreboard;
    ...
    function new (string name = "mcdf_checker", uvm_component parent);
      ...
      foreach(exp_bg_ports[i]) exp_bg_ports[i] = new($sformatf("exp_bg_ports[%d]",i),this);
