...
package mcdf_pkg;
    ...
    class mcdf_checker;
        ...
        task do_channel_disable_check(int id);
           forever begin
            ...
            if(this.chnl_vif[id].mon_ck.ch_ready === 1 && this.chnl_vif[id].mon_ck.ch_valid === 1)
    ...
    class mcdf_coverage;
		...
		covergroup cg_mcdf_reg_illegal_access;
			...
			cmdXaddrXdata: cross addr,cmd,wdata,rdata{
				...
				bins cmd_write_r_data = binsof(cmd.write) && binsof(addr.legal_r) && binsof(wdata.illegal);
			}
        endgroup
        ...
        covergroup cg_channel_disable;
            ...
            ch2_en: coverpoint mcdf_vif.mon_ck.chnl_en[2] {
                type_option.weight = 0;
                wildcard bins en = {1'b0};
                wildcard bins dis = {1'b1};
            }
            ...
            ch_enXch_vld: cross ch0_en,ch1_en,ch2_en,ch0_vld,ch1_vld,ch2_vld {
    ...
    class mcdf_base_test;
        ...
        virtual arb_intf arb_vif
            ...
            this.env.chker.set_interface('{ch0_vif,ch1_vif,ch2_vif},arb_vif,mcdf_vif);
    ...
    class mcdf_arbiter_coverage_test extends mcdf_base_test;
        function new(string name = "mcdf_arbiter_coverage_test");
            super.new(name);
        endfunction
        ...
        task do_reg();
            ...
            // 配置优先级，确保覆盖所有可能的优先级比较
            // 测试组合：0>1, 1>0, 0>2, 2>0, 1>2, 2>1
            wr_val = (1 << 3) + ($urandom_range(0,3) << 1) + 1;  // 通道0：len=1, prio=2, en=1
            write_reg(`SLV0_RW_ADDR, wr_val);
            
            wr_val = (2 << 3) + ($urandom_range(0,3) << 1) + 1;  // 通道1：len=2, prio=1, en=1
            write_reg(`SLV1_RW_ADDR, wr_val);
            
            wr_val = (3 << 3) + ($urandom_range(0,3) << 1) + 1;  // 通道2：len=3, prio=3, en=1
            write_reg(`SLV2_RW_ADDR, wr_val);
        ...
        task do_formatter();
            void'(fmt_gen.randomize() with {fifo == MED_FIFO; bandwidth == MED_WIDTH;});
            fmt_gen.start();
        endtask
        
        task do_data();
            // 测试1：通道0和通道1同时请求，通道2无效
            $display("=== Testing channels 0 & 1 with channel 2 disabled ===");
            void'(chnl_gens[0].randomize() with {ntrans == 50; ch_id == 0; data_nidles == 0; pkt_nidles == 1; data_size == 8;});
            void'(chnl_gens[1].randomize() with {ntrans == 50; ch_id == 1; data_nidles == 0; pkt_nidles == 1; data_size == 8;});
            // 禁用通道2
            void'(chnl_gens[2].randomize() with {ntrans == 0;});
            ...
            // 测试2：通道0和通道2同时请求，通道1无效
            $display("=== Testing channels 0 & 2 with channel 1 disabled ===");
            void'(chnl_gens[0].randomize() with {ntrans == 50; ch_id == 0; data_nidles == 0; pkt_nidles == 1; data_size == 8;});
            void'(chnl_gens[2].randomize() with {ntrans == 50; ch_id == 2; data_nidles == 0; pkt_nidles == 1; data_size == 16;});
            // 禁用通道1
            void'(chnl_gens[1].randomize() with {ntrans == 0;});
            ...
            // 测试3：通道1和通道2同时请求，通道0无效
            $display("=== Testing channels 1 & 2 with channel 0 disabled ===");
            void'(chnl_gens[1].randomize() with {ntrans == 50; ch_id == 1; data_nidles == 0; pkt_nidles == 1; data_size == 8;});
            void'(chnl_gens[2].randomize() with {ntrans == 50; ch_id == 2; data_nidles == 0; pkt_nidles == 1; data_size == 16;});
            // 禁用通道0
            void'(chnl_gens[0].randomize() with {ntrans == 0;});
