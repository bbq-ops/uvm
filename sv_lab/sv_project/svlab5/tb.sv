...
interface fmt_intf(input clk,input rstn);
    ...
    clocking drv_ck @(posedge clk);
        default input #1ns output #1ns;
        input fmt_chid,fmt_req,fmt_length,fmt_data,fmt_start,fmt_end;
        output fmt_grant;
    endclocking
    
    clocking mon_ck @(posedge clk);
        default input #1ns output #1ns;
        input fmt_chid,fmt_req,fmt_length,fmt_data,fmt_grant,fmt_start,fmt_end;
    endclocking
endinterface
...
module tb;
    ...
    initial begin
        ...
        tests["mcdf_arbiter_coverage_test"] = t6;
        
        if($value$plusargs("TESTNAME=%s",name)) begin
            if(tests.exists(name)) begin
                tests[name].set_interface(chnl0_if,chnl1_if,chnl2_if,reg_if,fmt_if,mcdf_if,arb_if);
                tests[name].run();
            end
            else begin
            ...
            tests["mcdf_data_consistence_basic_test"].set_interface(chnl0_if, chnl1_if, chnl2_if, reg_if, fmt_if, mcdf_if, arb_if);
