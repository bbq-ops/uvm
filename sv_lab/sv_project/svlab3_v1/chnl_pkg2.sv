...
package chnl_pkg2;
  ...
  class chnl_root_test;
    ...
    virtual function void do_config();
      assert(gen[0].randomize() with {ntrans==100;ch_id==0; data_nidles==0; pkt_nidles==1; data_size==8;})
      ...
      // data_size == 6
	  assert(gen[1].randomize() with {ntrans==50;ch_id==1;data_nidles inside{[1:2]};pkt_nidles inside {[3:5]};data_size==6;})
		else $fatal("[RNDFALL] gen[1] randomization failure!");

      // USER TODO 2.3
      // ntrans==80, data_nidles inside [0:1], pkt_nidles inside [1:2],
      // data_size == 32
	  assert(gen[2].randomize() with {ntrans==80;ch_id==2;data_nidles inside {[0:1]};pkt_nidles inside {[1:2]};data_size==32;})
		else $fatal("[RNDFALL] gen[1] randomization failure!");
    endfunction
  endclass
  ...
  class chnl_burst_test extends chnl_root_test;
	...
	function void do_config();
      assert(gen[0].randomize() with {ntrans inside {[80:100]}; ch_id==0; data_nidles==0; pkt_nidles==1; data_size inside {8,16,32};})
        else $fatal("[RNDFAIL] gen[0] randomization failure!");		
      assert(gen[1].randomize() with {ntrans inside {[80:100]}; ch_id==1; data_nidles==0; pkt_nidles==1; data_size inside {8,16,32};})
        else $fatal("[RNDFAIL] gen[1] randomization failure!");	
      assert(gen[2].randomize() with {ntrans inside {[80:100]}; ch_id==2; data_nidles==0; pkt_nidles==1; data_size inside {8,16,32};})
  ...
  class chnl_fifo_full_test extends chnl_root_test;
	...
	function void do_config();
      assert(gen[0].randomize() with {ntrans inside {[1000:2000]};ch_id==0;data_nidles==0; pkt_nidles==1; data_size inside {8,16,32};})
        else $fatal("[RNDFAIL] gen[0] randomization failure!");			
      assert(gen[1].randomize() with {ntrans inside {[1000:2000]};ch_id==1;data_nidles==0; pkt_nidles==1; data_size inside {8,16,32};})
        else $fatal("[RNDFAIL] gen[1] randomization failure!");	  
      assert(gen[2].randomize() with {ntrans inside {[1000:2000]};ch_id==2;data_nidles==0; pkt_nidles==1; data_size inside {8,16,32};})
        else $fatal("[RNDFAIL] gen[2] randomization failure!");	
	endfunction
	...
	task run_stop_callback();
      $display("run_stop_callback enterred");
		fork
			wait(agent[0].vif.drv_ck.ch_margin == 'h20);
			wait(agent[1].vif.drv_ck.ch_margin == 'h20);
			wait(agent[2].vif.drv_ck.ch_margin == 'h20);
		join
