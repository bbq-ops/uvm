...
begin
	...
	FMT_IDLE : 	begin
						if (cnt_rec_r == length_r - 2'd1) //need to decode and pay attention to cnt_rec_r start from 0 
							n_state = FMT_REQ;      //go to REQ state to handle last data
						else 
							n_state = FMT_IDLE;
					end 
					
	FMT_REQ : n_state = FMT_WAIT_GRANT; //package complete, wait for grant
...
begin
  if (!rstn_i)begin
    ...
    FMT_REQ : begin
      fmt_ack_r = 1'b0;  // package is full, stop receiving
      fmt_req_r = 1'b1;
      fmt_end_r = 1'b0;
      fmt_start_r = 1'b0;
      fmt_send_r = 1'b0;
